...
def get_col_major_tma_aligned_tensor(x: torch.Tensor, error_if_expensive: bool = True) -> torch.Tensor:
    """
    ...
    if error_if_expensive:
        raise ValueError('Transposing the LHS scaling tensor is too expensive, please ensure that x is column-major layout and 16-byte aligned along the M axis.')
